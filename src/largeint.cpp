...
LargeInt operator-(const LargeInt &intA, const LargeInt &intB)
{
    ...
    //if the result of subtracting digits was negative, add 10 and take one from the previous digit
    ...
    //if the result of the subtraction was negative (shouldn't be possible because comparison done earlier)
    ...
    LargeInt difAB{};
    difAB.m_shorts.clear(); //the default constructor could be changed to init to anything, clear to be safe

    //because tmp_vec_buf uses signed ints for convenience, do checks to ensure values can be made into unsigned shorts without narrowing
